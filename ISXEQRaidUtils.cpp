...
#include "ISXEQRaidUtils.h"

// The mandatory pre-setup function.  Our name is "ISXEQRaidUtils", and the class is ISXEQRaidUtils.
...
// our instanced class.
ISXPreSetup("ISXEQRaidUtils",ISXEQRaidUtils);
...
unsigned int ISXEQRaidUtilsXML=0;
...
bool ISXEQRaidUtils::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQRaidUtils Loaded");
return true;
}

// shutdown sequence
void ISXEQRaidUtils::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQRaidUtilsXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQRaidUtilsXML);
...
void ISXEQRaidUtils::ConnectServices()
{
...
void ISXEQRaidUtils::RegisterCommands()
{
    // add any commands
...
void ISXEQRaidUtils::RegisterAliases()
{
    // add any aliases
}

void ISXEQRaidUtils::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQRaidUtils::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQRaidUtils::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQRaidUtils::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQRaidUtils::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQRaidUtils::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQRaidUtilsService)
    //        pISInterface->ShutdownService(this,hISXEQRaidUtilsService);
}

void ISXEQRaidUtils::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQRaidUtilsXML
    ISXEQRaidUtilsXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQRaidUtilsXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQRaidUtilsXML,"General"))
    {
